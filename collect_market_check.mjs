...
const root=new URL('./',import.meta.url),packet=new URL(process.argv[2].replace(/\/$/,'')+'/',root);fs.mkdirSync(packet,{recursive:true});
const read=p=>JSON.parse(fs.readFileSync(new URL(p,root),'utf8'));
const c=read('notebook-evidence/home.json').contracts,prior=read(process.argv[3]+'/tape.json');
const raw=[];let id=0;
async function rpc(method,params){
 for(let attempt=0;attempt<3;attempt++){
  await new Promise(r=>setTimeout(r,attempt?3000:1200));const request={jsonrpc:'2.0',id:++id,method,params};let response;
  try{const res=await fetch('https://rpc.mainnet.chain.robinhood.com',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(request),signal:AbortSignal.timeout(30000)});try{response=await res.json();}catch{response={error:{message:'Non-JSON HTTP '+res.status}};}}catch(e){response={error:{message:String(e)}};}
...
const focused=read('buyer-investigation/targets.json').addresses;
const erc=parseAbi(['function balanceOf(address) view returns(uint256)']);
for(const address of focused){
 items.push({label:'wallet.'+address,address:c.standard,abi:tokenABI,functionName:'balanceOf',args:[address]});
 items.push({label:'cash.'+address+'.ETH',address:c.multicall3,abi:mc,functionName:'getEthBalance',args:[address]});
 for(const [name,asset] of Object.entries({WETH:'0x0bd7d308f8e1639fab988df18a8011f41eacad73',USDG:'0x5fc5360d0400a0fd4f2af552add042d716f1d168'}))items.push({label:'cash.'+address+'.'+name,address:asset,abi:erc,functionName:'balanceOf',args:[address]});
}
const bankABI=read('notebook-evidence/bank-abi.json');
for(const functionName of ['totalBranches','totalPendingLive'])items.push({label:'bank.'+functionName,address:c.centralBank,abi:bankABI,functionName});
