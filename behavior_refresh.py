...
def run(root=ROOT):
    new=root/'behavior-evidence';out=root/'behavior-results'/'participants'
    ...
    with tempfile.TemporaryDirectory(prefix='standard-reentry-') as tmp:
        ...
        history=read(root/'strategy-current/charter-transfer-history.json')
        history['response']['result']+=read(new/'nft-delta.json')['result']
        put('charter-transfer-history.json',history)
