...
const root=new URL('./',import.meta.url),dir=new URL('behavior-evidence/',root);
const read=p=>JSON.parse(fs.readFileSync(new URL(p,root),'utf8'));
const target=read('behavior-evidence/target.json'),tag=target.header.number;
const state=Object.fromEntries(read('behavior-evidence/state.json').map(x=>[x.label,x.value]));
...
const nft=await rpc('eth_getLogs',[{address:c.charterNFT,topics:['0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef'],fromBlock:toHex(target.priorHolderBlock+1),toBlock:tag}]);
if(nft.error)throw Error(JSON.stringify(nft.error));fs.writeFileSync(new URL('nft-delta.json',dir),JSON.stringify(nft));
